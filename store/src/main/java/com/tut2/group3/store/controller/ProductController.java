...
public class ProductController {
    ...
    public Result<List<ProductDto>> getAllProducts() {
        ...
        try {
            ...
            // Warehouse service is down or unreachable
            log.error("Warehouse service unavailable: {}", e.getMessage());
            return Result.error(503, "Sorry, the service is currently unavailable. Please try again later.");
        } catch (Exception e) {
            log.error("Error retrieving products: {}", e.getMessage(), e);
            return Result.error(500, "Sorry, the service is currently unavailable. Please try again later.");
        }
    }
}
